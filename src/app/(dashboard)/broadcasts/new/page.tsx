...
function NewBroadcastPageInner() {
  ...
  // Pre-fill from collection if ?collectionId= is in the URL
  useEffect(() => {
    const cid = searchParams.get('collectionId');
    if (!cid) return;
    // Use a local ref to track whether collection data has been loaded,
    // so we only set state after the async load completes — not
    ...
    async function loadCollection() {
      ...
      setCollectionId(cid);
      setName(`Collection: ${col.name}`);

      // Pre-fill template variables from collection data
      const prefillVars: Record<
        string,
        { type: 'static' | 'field' | 'custom_field'; value: string }
      > = {};
      prefillVars['1'] = { type: 'static', value: col.name };
      if (col.description) {
        prefillVars['2'] = { type: 'static', value: col.description };
      }
      setVariables(prefillVars);

      // Pre-fill header media from first image
      ...
      if (images && images.length > 0) {
        setHeaderMediaUrl(images[0].image_url);
      }
    }
    loadCollection();
    return () => {
